...
void compararAlgoritmosSinExactoVariandoNMCompleto(int cantInstanciasPorM, int minN, int maxN, int saltarDeA) {
    ...
    a_file << "n, m, fronteraExacto, fronteraHConstructiva, fronteraBLocal, fronteraGrasp, tiempoHConstructiva, tiempoBLocal, tiempoGrasp" << std::endl;
//    std::cout << "Variando m: {n=" << constanteN << "} => " << minM << " <= m <= " << maxM << std::endl;
    for (int i = minN; i <= maxN; i+=saltarDeA) {
        ...
        int fronteraReal = std::floor(float(i/2))*std::ceil((float) i/2);
        tiempoTotalHConstructiva = tiempoTotalHConstructiva/ cantInstanciasPorM;
        tiempoTotalBLocal = tiempoTotalBLocal / cantInstanciasPorM;
        tiempoTotalGrasp = tiempoTotalGrasp/ cantInstanciasPorM;
        promFronteraHConstructiva = promFronteraHConstructiva/ cantInstanciasPorM;
        promFronteraBLocal = promFronteraBLocal / cantInstanciasPorM;
        promFronteraGrasp = promFronteraGrasp/ cantInstanciasPorM;
        std::cout << i << ", "<<  m << ", " << fronteraReal << ", " << promFronteraHConstructiva << ", " << promFronteraBLocal << ", " << promFronteraGrasp << ", " << tiempoTotalHConstructiva << ", " << tiempoTotalBLocal << ", " << tiempoTotalGrasp << std::endl ;
        a_file << i << ", "<<  m << ", " << fronteraReal << ", " << promFronteraHConstructiva << ", " << promFronteraBLocal << ", " << promFronteraGrasp << ", " << tiempoTotalHConstructiva << ", " << tiempoTotalBLocal << ", " << tiempoTotalGrasp << std::endl ;
    }
...
void compararAlgoritmosSinExactoVariandoNCasoMaloHConstructivaBuenoBLocal(int cantInstanciasPorN, int minN, int maxN, int saltarDeA) {
    std::string nombreArchivo = "comp-algoritmos-sin-exacto-variando-n-caso-malo-hconstructiva-bueno-blocal";

    std::stringstream ss;
    ss <<  "/home/jscherman/CLionProjects/algo3-tp3-cmf/datos/" << nombreArchivo << ".csv";
    std::ofstream a_file (ss.str());

    a_file << "n, fronteraExacto, fronteraHConstructiva, fronteraBLocal, fronteraGrasp, tiempoHConstructiva, tiempoBLocal, tiempoGrasp" << std::endl;
//    std::cout << "Variando m: {n=" << constanteN << "} => " << minM << " <= m <= " << maxM << std::endl;
    for (int i = minN; i <= maxN; i+=saltarDeA) {
        long long tiempoTotalHConstructiva = 0;
        long long tiempoTotalBLocal = 0;
        ...
        for (int j = 0; j < cantInstanciasPorN; ++j) {
            std::vector<std::list<int>> listaAdyacencias = Utils::genCasoMaloHConstructivaBuenoBLocal(i);
            std::vector<std::vector<bool>> matrizAdyacencias = Utils::aMatrizAdyacencias(listaAdyacencias);
            {
                // H. Constructiva
                auto tpi = std::chrono::high_resolution_clock::now();
        ...
        int fronteraReal = std::floor(float(i/2))*std::ceil((float) i/2) + 3;
        ...
        promFronteraHConstructiva = promFronteraHConstructiva/ cantInstanciasPorN;
        promFronteraBLocal = promFronteraBLocal / cantInstanciasPorN;
        promFronteraGrasp = promFronteraGrasp/ cantInstanciasPorN;
        std::cout << i <<  ", " << fronteraReal << ", " << promFronteraHConstructiva << ", " << promFronteraBLocal << ", " << promFronteraGrasp << ", " << tiempoTotalHConstructiva << ", " << tiempoTotalBLocal << ", " << tiempoTotalGrasp << std::endl ;
        a_file << i << ", " << fronteraReal << ", " << promFronteraHConstructiva << ", " << promFronteraBLocal << ", " << promFronteraGrasp << ", " << tiempoTotalHConstructiva << ", " << tiempoTotalBLocal << ", " << tiempoTotalGrasp << std::endl ;
    }
    a_file.close();
    std::cout << "Listo!" << std::endl;
}
...
//    compararAlgoritmosSinExactoVariandoKCasoMaloTodos(3, 5, 1500, 20);
    compararAlgoritmosSinExactoVariandoNCasoMaloHConstructivaBuenoBLocal(3, 5, 1500, 20);
    return 0;
}
